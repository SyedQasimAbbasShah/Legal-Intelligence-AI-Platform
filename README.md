...
8. **Run the full end-to-end demo**
   ...
   ```
   This exercises all 13 endpoints against a live running server. During development, running
   this end-to-end surfaced a real bug: `/audit/summary` returned a 500 error whenever the
   audit database was unreachable (no graceful fallback, unlike every other engine). Fixed in
   `audit.py` — confirmed all 13 endpoints now return 200 against a live server.
